...
import tv.joyplus.backend.report.memcache.MemCached;
import tv.joyplus.backend.utility.CommonUtility;
import tv.joyplus.backend.utility.Const;

public class JobResultDaoImplWithCache extends JdbcDaoSupport implements JobResultDao {

	protected Log log = LogFactory.getLog(JobResultDaoImplWithCache.class);
	...
	public void  saveJobResults(List<JobResultDto> results) {
		...
		while (itResult.hasNext()) {
			...
			List<Map<String, Object>> lstCampaignResult = (List<Map<String, Object>>) memCache.get(business_id+Const.CACHE_DVERTISER_CAMPAIGN_NAME_BY_CAMPAIGNID+jobResultRow.campaign_id);
			if(lstCampaignResult == null){
				lstCampaignResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.campaign_id);
				memCache.add(business_id+Const.CACHE_DVERTISER_CAMPAIGN_NAME_BY_CAMPAIGNID+jobResultRow.campaign_id, lstCampaignResult);
			}
			...
			List<Map<String, Object>> lstCreativeResult = (List<Map<String, Object>>) memCache.get(business_id+Const.CACHE_AD_UNITS_BYID+jobResultRow.adv_id);
			if(lstCreativeResult==null){
				lstCreativeResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.adv_id);
				memCache.add(business_id+Const.CACHE_AD_UNITS_BYID+jobResultRow.adv_id, lstCreativeResult);
			}
			...
			List<Map<String, Object>> lstPublicationResult = (List<Map<String, Object>>) memCache.get(business_id+Const.CACHE_PUBLICATIONS_BYID+jobResultRow.publication_id);
			if(lstPublicationResult==null){
				lstPublicationResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.publication_id);
				memCache.add(business_id+Const.CACHE_PUBLICATIONS_BYID+jobResultRow.publication_id, lstPublicationResult);
			}
			...
			List<Map<String, Object>> lstZoneResult = (List<Map<String, Object>>) memCache.get(business_id+Const.CACHE_ZONES_BYID+jobResultRow.zone_id);
			if(lstZoneResult==null){
				lstZoneResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.zone_id);
				memCache.add(business_id+Const.CACHE_ZONES_BYID+jobResultRow.zone_id, lstZoneResult);
			}
...
//			List<Map<String, Object>> lstRegionResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.region_code);
			List<Map<String, Object>> lstRegionResult = (List<Map<String, Object>>) memCache.get(business_id+Const.CACHE_REGIONAL_TARGETING_BYCODE+jobResultRow.region_code);
			if(lstRegionResult==null){
				lstRegionResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.region_code);
				memCache.add(business_id+Const.CACHE_REGIONAL_TARGETING_BYCODE+jobResultRow.region_code, lstRegionResult);
			}
...
//			List<Map<String, Object>> lstDeviceResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.device_name);
			List<Map<String, Object>> lstDeviceResult = (List<Map<String, Object>>) memCache.get(business_id+Const.CACHE_DEVICE_DATA_BYNAME+jobResultRow.device_name);
			if(lstDeviceResult==null){
				lstDeviceResult = getJdbcTemplate().queryForList(strSQL, jobResultRow.device_name);
				memCache.add(business_id+Const.CACHE_DEVICE_DATA_BYNAME+jobResultRow.device_name, lstDeviceResult);
			}
	...
	public MemCached getMemCache() {
		return memCache;
	}

	public void setMemCache(MemCached memCache) {
